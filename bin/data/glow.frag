...
void main()
{
    // ---------------- First, set up the camera rays for ray marching ----------------
        ...
        vec2 uv = gl_FragCoord.xy/iResolution.xy * 10.0  - 1.5;
        uv -= vec2(0.5);
        uv = rotate2d( sin(PI/2.)*iGlobalTime*u_02 ) * uv;
        uv += vec2(1.5);
        // Camera up vector.
        ...
        float mx=0.2*PI*2.0 + iGlobalTime * 0.01;
        float my=-0.5*10.0 + sin(iGlobalTime * 0.03)*0.2+0.2;//*PI/2.01;
    ...
    gl_FragColor = vec4(sqrt(clamp(semiFinal, 0.0, 1.0)),1.);
}
